...
from .models import ArticuloDepartamento, Departamento, Funcionario, Cargo, DetalleCostoDepartamento, CostoDepartamento, Reserva, Carro
...
from .forms import ArticuloDepartamentoForm, DepartamentoForm, ImagenDepartamentoForm, FuncionarioForm, DetalleCostoDepartamentoForm, CarroForm
...
from .utils import get_or_set_reserva_session
...
class ListadoDepartamentosDisponibles(LoginRequiredMixin, IsMixinDate, ListView):
	...
	def get_queryset(self):
		queryset = self.model.objects.filter(dep_estado = True)
		return queryset
...
class DetalleDepartamentosDisponibles(LoginRequiredMixin, IsMixinDate, FormView):
	template_name = 'DetalleDepartamentos.html'
	form_class = CarroForm

	def get_object(self):
		return get_object_or_404(Departamento, slug=self.kwargs["slug"])

	def get_success_url(self):
		return reverse("carro")

	def form_valid(self, form):
		carro_f_res = get_or_set_reserva_session(self.request)
		carro_f_dep = self.get_object()

		item_filter = carro_f_res.items.filter(carro_f_dep=carro_f_dep)

		if item_filter.exists():
			item = item_filter.first()
			item.carro_cantidad = int(form.cleaned_data['carro_cantidad'])
			item.save()
		else:
			new_item = form.save(commit=False)
			new_item.carro_f_dep = carro_f_dep
			new_item.carro_f_res = carro_f_res
			new_item.save()


		return super(DetalleDepartamentosDisponibles, self).form_valid(form)

	def get_context_data(self, **kwargs):
		context = super(DetalleDepartamentosDisponibles, self).get_context_data(**kwargs)
		context['departamento'] = self.get_object()
		context['title'] = 'Departamento'
		return context

class CarroView(LoginRequiredMixin, IsMixinDate, TemplateView):
	...
	def get_form_kwargs(self):
		kwargs = super(CarroView, self).get_form_kwargs()
		kwargs["user_id"] = self.request.user.id
		return kwargs

	def get_context_data(self, **kwargs):
		context = super(CarroView, self).get_context_data(**kwargs)
		context['reserva'] = get_or_set_reserva_session(self.request)
		return context

class IncrementarDiasView(LoginRequiredMixin, IsMixinDate, View):
	def get(self, request, *args, **kwargs):
		carro_f_res_item = get_object_or_404(Carro, carro_id=kwargs['pk'])	
		carro_f_res_item.carro_cantidad +=1
		carro_f_res_item.save()
		return redirect("carro")

class ReducirDiasView(LoginRequiredMixin, IsMixinDate, View):
	def get(self, request, *args, **kwargs):
		carro_f_res_item = get_object_or_404(Carro, carro_id=kwargs['pk'])
		if carro_f_res_item.carro_cantidad <=1:
			carro_f_res_item.delete()
		else:	
			carro_f_res_item.carro_cantidad -=1
			carro_f_res_item.save()
		return redirect("carro")
...
class RemoverReservaView(LoginRequiredMixin, IsMixinDate, View):
	def get(self, request, *args, **kwargs):
		carro_f_res_item = get_object_or_404(Carro, carro_id=kwargs['pk'])
		carro_f_res_item.delete()
		return redirect("carro")

class PagoView(LoginRequiredMixin, IsMixinDate, TemplateView):
    template_name = 'Pago.html'

    def get_context_data(self, **kwargs):
        context = super(PagoView, self).get_context_data(**kwargs)
        context['PAYPAL_SANDBOX_CLIENT_ID'] = settings.PAYPAL_SANDBOX_CLIENT_ID
        return context
...
class DepartamentoListView(LoginRequiredMixin, ValidatePermissionRequiredMixin, IsMixinDate, ListView):
	...
	permission_required = 'Servicio_Turismo_Real.view_departamento'
...
class DepartamentoCreateView(LoginRequiredMixin, ValidatePermissionRequiredMixin, CreateView):
    ...
    permission_required = 'Servicio_Turismo_Real.create_departamento'
...
class DepartamentoUpdateView(LoginRequiredMixin, ValidatePermissionRequiredMixin, UpdateView):
	...
	template_name = 'Crear-Departamento.html'
	success_url = reverse_lazy('listadodepartamento')
	permission_required = 'Servicio_Turismo_Real.change_departamento'
...
class DepartamentoDeleteView(LoginRequiredMixin, ValidatePermissionRequiredMixin, DeleteView):
	...
	success_url = reverse_lazy('listadodepartamento')
	permission_required = 'Servicio_Turismo_Real.delete_departamento'
