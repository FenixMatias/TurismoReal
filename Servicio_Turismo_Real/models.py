...
class Departamento(BaseModel):
    ...
    dep_acondicionado = models.BooleanField(verbose_name='Acondicionado', default = False)
    dep_canon_renta = models.PositiveIntegerField(verbose_name='Renta', default=0)
    dep_estado = models.BooleanField(verbose_name='Disponibilidad', default = False)
    ...
    def get_dep_canon_renta(self):
        return"{:20,}".format(self.dep_canon_renta)
    ...
    def get_absolute_url(self):
        return reverse("detalledepartamentosdisponibles", kwargs={'slug': self.slug})
...
class Carro(models.Model):
    ...
    carro_cantidad = models.PositiveIntegerField(verbose_name='Cantidad de Dias', default=1)
    carro_f_dep = models.ForeignKey(Departamento, on_delete=models.CASCADE)
    carro_f_res = models.ForeignKey("Reserva", related_name='items', on_delete=models.CASCADE)

    def __str__(self):
        return f"{self.carro_cantidad} x {self.carro_f_dep.dep_nombre}"

    def get_raw_total_item_price(self):
        return self.carro_cantidad * self.carro_f_dep.dep_canon_renta

    def get_total_item_price(self):
        dep_canon_renta = self.get_raw_total_item_price()
        return"{:20,}".format(dep_canon_renta)
...
class Pago(models.Model):
    ...
    pago_metodo = models.CharField(max_length=20, choices=(('Paypal', 'Mercado_pago'),), verbose_name='Pagos')
    pago_fecha = models.DateTimeField(verbose_name='Fecha Pago', auto_now_add=True)
    pago_efectuado = models.BooleanField(verbose_name='Pago Realizado', default=False)
    pago_monto = models.FloatField()
    pago_respuesta = models.TextField()
    pago_f_res = models.ForeignKey("Reserva", on_delete=models.CASCADE, verbose_name='Pagos')
    ...
    def numero_referencia(self):
        return f"Pago-{self.reserva}-{self.pk}"
...
class Reserva(models.Model):
    ...
    res_fecha = models.DateTimeField(verbose_name='Fecha de Reserva', blank=True, null=True)
    res_estado = models.BooleanField(verbose_name='Reserva', default=False)
    f_use = models.ForeignKey(User, blank=True, null=True, on_delete=models.CASCADE)
    ...
    def get_raw_subtotal(self):
        total = 0
        for carro_f_res_item in self.items.all():
            total += carro_f_res_item.get_raw_total_item_price()
        return total

    def get_subtotal(self):
        subtotal = self.get_raw_subtotal()
        return"{:20,}".format(subtotal)
...
def pre_save_departamento_receiver(sender, instance, *args, **kwargs):
    if not instance.slug:
        instance.slug = slugify(instance.dep_nombre)

pre_save.connect(pre_save_departamento_receiver, sender=Departamento)
